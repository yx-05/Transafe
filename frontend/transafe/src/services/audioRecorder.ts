export class CallAudioStreamer {
  ...
  private audioQueue: Uint8Array<ArrayBuffer>[] = [];
  ...
  private enqueuePeerAudioChunk(arrayBuffer: ArrayBuffer) {
    const chunk = new Uint8Array(arrayBuffer.slice(0));
  ...
  /**
   * Mute/unmute the captured microphone in place.
   *
   * Disabling the MediaStream audio track stops the MediaRecorder from
   * producing audio chunks, so no audio is relayed to the peer while muted.
   * Does NOT stop the stream — toggling back on resumes from where it left.
   */
