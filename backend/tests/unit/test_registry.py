...
def test_publish_artifact_survives_insert_failure(mock_client: MagicMock) -> None:
    ...
    mock_table.insert.return_value.execute.side_effect = RuntimeError("relation does not exist")
    mock_client.return_value.table.return_value = mock_table

    assert publish_artifact("SCAM-027", "pack", "campaign_pack", "phone_worker", "{}") is None
...
def test_get_artifact_missing_table_returns_none(mock_client: MagicMock) -> None:
    ...
    mock_client.return_value.table.side_effect = RuntimeError("relation does not exist")
...
@patch("src.enterprise.registry.get_supabase_client")
def test_list_artifacts_missing_table_returns_empty(mock_client: MagicMock) -> None:
...
@patch("src.enterprise.registry.get_supabase_client")
def test_rollback_artifact_publishes_old_version(mock_client: MagicMock) -> None:
...
@patch("src.enterprise.registry.get_supabase_client")
def test_rollback_unknown_version_returns_none(mock_client: MagicMock) -> None:
...
@patch("src.enterprise.registry.get_supabase_client")
def test_update_effectiveness_writes_metric(mock_client: MagicMock) -> None:
...
def test_publish_accepts_injected_client() -> None:
    ...
    mock_table = MagicMock()
    mock_table.select.return_value.eq.return_value.order.return_value.limit.return_value.execute.return_value.data = []
    mock_table.insert.return_value.execute.return_value.data = [{"id": "art-9", "version": 1}]
    injected = MagicMock()
    injected.table.return_value = mock_table

    result = publish_artifact(
        "SCAM-030", "pack", "campaign_pack", "phone_worker", "{}", client=injected
    )
    assert result is not None
    assert result["version"] == 1
